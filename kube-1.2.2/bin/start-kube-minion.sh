...
KUBE_PROXY=$YAAS_SERVERDIR/kube-proxy
KUBELET=$YAAS_SERVERDIR/kubelet
...
if [ -z "$KUBE_PROXY_LOG_FILE" ]; then
    KUBE_PROXY_LOG_FILE="$YAAS_LOGDIR/kube-proxy.log"
fi
mkdir -p "$(dirname $KUBE_PROXY_LOG_FILE)"
log "Starting kube-proxy ..."
"$KUBE_PROXY" --master=http://$YAAS_MASTER:8080 --proxy-mode=userspace --logtostderr=true >> $KUBE_PROXY_LOG_FILE 2>&1 &
sleep 2
...
if [ -z "$KUBELET_LOG_FILE" ]; then
    KUBELET_LOG_FILE=$YAAS_LOGDIR/kubelet.log
fi
