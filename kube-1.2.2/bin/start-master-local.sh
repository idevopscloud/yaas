...
KUBE_APISERVER=$YAAS_SERVERDIR/kube-apiserver
KUBE_CM=$YAAS_SERVERDIR/kube-controller-manager
KUBE_SCHEDULER=$YAAS_SERVERDIR/kube-scheduler

if [ -z "$KUBE_API_LOG_FILE" ]; then
    KUBE_API_LOG_FILE="$YAAS_LOGDIR/kube-apiserver.log"
fi

if [ -z "$KUBE_SCH_LOG_FILE" ]; then
    KUBE_SCH_LOG_FILE="$YAAS_LOGDIR/kube-scheduler.log"
fi

if [ -z "$KUBE_CM_LOG_FILE" ]; then
    KUBE_CM_LOG_FILE="$YAAS_LOGDIR/kube-controller-manager.log"
fi

# check if current host is the master
if [ $YAAS_MASTER != "$MY_IP" ]; then
...
#if [ -f "$YAAS_ROOT/conf/scheduler-policy-config.json" ];
#then
#    scheduler_policy_file="$YAAS_ROOT/conf/scheduler-policy-config.json"
#fi
