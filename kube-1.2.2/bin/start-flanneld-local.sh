...
FLANNELD=$YAAS_SERVERDIR/flanneld
FLANNELD_LOG_FILE="$YAAS_LOGDIR/flanneld.log"
...
if [[ $(curl -L http://${etcd_hosts[0]}:4001/v2/keys/coreos.com/network/config 2>/dev/null | grep "Key not found") ]]; then
    echo "curl -L http://${etcd_hosts[0]}:4001/v2/keys/coreos.com/network/config -XPUT --data-urlencode value@${YAAS_CONFDIR}/flannel-config.json > /dev/null 2>&1"
    curl -L http://${etcd_hosts[0]}:4001/v2/keys/coreos.com/network/config -XPUT --data-urlencode value@${YAAS_CONFDIR}/flannel-config.json > /dev/null 2>&1
fi
